...
export async function signUp({ email, password, name }) {
  ...
  if (data.user) {
    await supabase.from('profiles').upsert({
      ...
      name,
      email,
      bio:        'Olá! Estou de volta no Orkut :)',
      country:    'Brasil',
      rel_status: 'solteiro(a)',
...
export async function sendRecado(fromId, toId, text) {
  const { error } = await supabase
    .from('recados').insert({ from_id: fromId, to_id: toId, text })
  if (error) throw error
}

export async function deleteRecado(recadoId, userId) {
  await supabase.from('recados')
    .delete().eq('id', recadoId).eq('to_id', userId)
}
...
export async function sendDepoimento(fromId, toId, text) {
  const { error } = await supabase
    .from('depoimentos').insert({ from_id: fromId, to_id: toId, text })
  if (error) throw error
}
...
export async function getCommunities() {
  const { data } = await supabase
    .from('communities').select('*').order('members_count', { ascending: false })
  return data || []
}
...
export async function createCommunityPost(authorId, communityId, text) {
  const { error } = await supabase
    .from('community_posts').insert({ author_id: authorId, community_id: communityId, text })
  if (error) throw error
}
...
export async function sendMessage(fromId, toId, text) {
  const { error } = await supabase
    .from('messages').insert({ from_id: fromId, to_id: toId, text })
  if (error) throw error
}
